...
function calendar() {
    var events = boilerplate.loopValues(boilerplate.sliceValues(boilerplate.getValues(), 1));
...
var doGet = function(e) {
  try {
    ...
    var thisFunc = parE && parE.parameter["func"]? this[libName][parE.parameter["func"]].name:"mis";
    console.log("The received object[func] ", thisFunc);

    var thisArgs = parE && parE.parameter["args"]? parE.parameter["args"]:this[libName].getScriptUrl();
    ...
    if (typeof this[libName]["freeSBD"] == "function") {
      ...
      console.log("The result of applying doGet code on the received object parameters ", rawResult.getContent())
      return rawResult;
    }
    else {
    ...
    throw new Error("Error executing function this[libName].freeSBD(" + [thisFunc?.name, thisArgs] + ")")
  };
}
